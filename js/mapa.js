...
function normalizarStatus(status) {
  ...
  return status
    ...
    .toLowerCase()
    .replace(/[–—]/g, "-")               // padroniza travessões como hífen
    .replace(/\s*-\s*/g, " - ")        // padroniza espaços antes/depois do hífen
    .replace(/\s+/g, " ")               // remove espaços duplicados
    .trim();
}

function statusEmAnalise(status) {
  var statusNorm = normalizarStatus(status);
  ...
  return termosEmAnalise.includes(statusNorm);
}

function statusEmTramitacaoSEINFRA(status) {
  return normalizarStatus(status) === "em tramitacao - seinfra";
}

function corPorStatus(status) {
  var statusNorm = normalizarStatus(status);

  if (!statusNorm) return "#808080";

  if (statusNorm.includes("aprovado")) return "#2ecc71";   // verde
  if (statusEmAnalise(status)) return "#f1c40f";            // amarelo
  if (statusNorm.includes("pendente")) return "#e67e22";   // laranja
  if (statusNorm.includes("iniciar")) return "#3498db";    // azul
  if (statusEmTramitacaoSEINFRA(status)) return "#9b59b6"; // roxo

  return "#808080"; // padrão
}

// ============================
// FUNÇÕES AUXILIARES - DOCUMENTOS DO POPUP
// ============================

function caminhoDocumentoValido(caminho) {
  if (!caminho) return false;

  var valor = caminho.toString().trim();
  if (!valor) return false;

  var valorNorm = normalizarStatus(valor);
  ...
  return !valoresInvalidos.includes(valorNorm);
}

function montarLinkDocumento(rotulo, caminho, emoji) {
  if (!caminhoDocumentoValido(caminho)) return "";

  return `
    <div style="margin-top:6px;">
      <a href="${caminho}" download>${emoji} Baixar ${rotulo}</a>
    </div>
  `;
}

function montarSecaoDocumentos(documentos) {
  var htmlDocumentos = "";

  documentos.forEach(function (doc) {
    htmlDocumentos += montarLinkDocumento(doc.rotulo, doc.caminho, doc.emoji);
  });

  if (!htmlDocumentos) return "";

  return `
    <div style="margin-top:8px; padding-top:6px; border-top:1px solid #ddd;">
      <div class="popup-valor"><span class="popup-label">Documentos disponíveis:</span></div>
      ${htmlDocumentos}
    </div>
  `;
}
  ...
  .then(function (dados) {
    ...
    var geojsonLayer = L.geoJSON(dados, {
      ...
      onEachFeature: function (feature, layer) {
        ...
        var documentos = [
          {
            rotulo: "Licença Prévia",
            caminho: p.pdf_licenca_previa,
            emoji: "📄"
          },
          {
            rotulo: "Licença de Instalação",
            caminho: p.pdf_licenca_instalacao,
            emoji: "🏗️"
          },
          {
            rotulo: "Parecer Técnico",
            caminho: p.pdf_parecer_licenca,
            emoji: "📝"
          },
          {
            rotulo: "Autorização de Extirpação",
            caminho: p.pdf_autorizacao_extirpacao,
            emoji: "🌳"
          },
          {
            rotulo: "Portaria de Outorga",
            caminho: p.pdf_portaria_outorga,
            emoji: "💧"
          },
          {
            rotulo: "Autorização ANAC",
            caminho: p.pdf_autorizacao_anac,
            emoji: "✈️"
          }
        ];
        ...
        var htmlPopup = `
          <div>
            ...
            ${montarSecaoDocumentos(documentos)}
          </div>
        `;
...
function atualizarMapaELista() {
  ...
  featuresData.forEach(function (item) {
    ...
    if (statusSelecionado !== "todos") {
      var statusSelecionadoNorm = normalizarStatus(statusSelecionado);
      var statusNormFiltro = normalizarStatus(status);

      if (statusSelecionado === "em-analise" || statusSelecionadoNorm === "em analise") {
        if (!statusEmAnalise(status)) {
          inclui = false;
        }
      } else if (statusSelecionado === "em-tramitacao" || statusSelecionadoNorm === "em fase de tramitacao") {
        if (!statusEmTramitacaoSEINFRA(status)) {
          inclui = false;
        }
      } else if (statusNormFiltro !== statusSelecionadoNorm) {
        inclui = false;
      }
    }
    ...
    if (inclui) {
      ...
      if (statusNorm.includes("aprov")) {
        contAprovado++;
      } else if (statusEmAnalise(status)) {
        contEmAnalise++;
      } else if (statusEmTramitacaoSEINFRA(status)) {
        contEmTramitacao++;
  ...
  var elEmTramitacao = document.getElementById("contador-em-tramitacao");
  ...
  if (elEmTramitacao) elEmTramitacao.textContent = contEmTramitacao;
...
function adicionarLegenda() {
  ...
  legenda.onAdd = function () {
    ...
    div.innerHTML = `
      ...
      <div class="legenda-item">
        <span class="legenda-cor" style="background:#9b59b6;"></span> Em tramitação
      </div>
      <div class="legenda-item">
...
function atualizarDashboard(aprovado, analise, pendente, iniciar, tramitacao) {
  ...
  graficoStatus = new Chart(ctx, {
    type: "bar",
    data: {
      labels: ["Aprovado", "Em análise", "Pendente", "A iniciar", "Em tramitação"],
      datasets: [{
        label: "Quantidade",
        data: [aprovado, analise, pendente, iniciar, tramitacao],
        backgroundColor: [
          "#2ecc71",
          "#f1c40f",
          "#e67e22",
